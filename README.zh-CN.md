...
把 Vue 应用从一套组件库迁移到另一套（例如 **Angular + DevUI / Element Plus → Vue 3 + 你的 UI 库**）——并且尽量不出事。

## 诞生背景

这套工具来自一次真实的**跨框架重写**——把一个老的 **Angular + DevUI** 项目迁移到 **Vue 3** 的另一套组件库上。当时靠"把标签查找替换 + AI / codemod 映射建议"，总会漏下一些很隐蔽的破坏，往往到线上才暴露。这里的工具就是我们当时最希望手里有的护栏——从真正踩过的坑里沉淀出来。

## 解决的痛点

组件库迁移会以两种很隐蔽、普通查找替换和"按名映射"工具都抓不到的方式出问题：

1. **名字换对了，用法没对上** —— 标签能对上，但某个必填 prop / 数据约定没对上，而且改代码之前没人拦住。两个典型：
   - **树渲染出来是空白**：新库读 `label` / `children` 用的是另一个 prop（`:props`），跟旧库不一样；
   - **分类搜索被错换成高级搜索**：两者必填 prop 不同，界面渲染成乱码。
   这类问题**有静态特征** → **契约闸门**在改代码阶段、还没跑起来就拦下。
2. **组件用对了 ≠ 行为和旧版一致** —— 即便每个标签都映射正确，新版仍可能回归：按钮点了没反应、少发了某个 `/api` 请求、少了某个功能、特定数据下崩溃。这类问题**没有静态特征** → 只有把跑起来的新版和旧版（当基准 oracle）对比才能抓 → 这就是**语义差异**。

为什么不能只信映射工具 / AI 建议？按名映射**不懂语义等价、看不到你的运行时数据、也不校验自己的输出**——它会很自信地指给你一个根本不存在的组件，或者错误的等价物。这套工具把「相信映射」换成「用确定性检查 + 旧版当基准来验证」。
